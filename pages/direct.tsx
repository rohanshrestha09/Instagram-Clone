...
const Messenger: NextPage<Props> = ({ db }) => {
  const [chatRoomToggle, setChatRoomToogle] = useState<boolean>(false);
  const [getIndex, setGetIndex] = useState<number>(0);
  const [ioMessage, setIoMessage] = useState<string[]>([...db[getIndex].mesg]);
  ...
  const formSubmission = async (e: React.SyntheticEvent): Promise<void> => {
    e.preventDefault();
    setIoMessage(["...", message, ...ioMessage]);
    setMessageData({ message: "" });
  };

  useEffect(() => {
    setIoMessage([...db[getIndex].mesg]);
  }, [getIndex]);

  return (
    <div className="font-sans w-screen overflow-y-scroll">
      <Head>
      ...
      <Nav />
      <div className="w-full min-h-[91.2vh] flex items-center justify-center px-0 md:px-4">
        <div className="md:w-[58.5rem] w-full md:h-[96%] h-full grid grid-cols-8 md:border-2 rounded-md">
          <MessagePreview
            chatRoomToggle={chatRoomToggle}
            setChatRoomToggle={setChatRoomToogle}
            getIndex={getIndex}
...
export const getStaticProps: GetStaticProps = async () => {
  const db: { name: string; imgurl: string; mesg: string[] }[] = [
    {
